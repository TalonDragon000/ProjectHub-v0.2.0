          ...
          <div className="flex-1 overflow-y-auto space-y-6 pb-24 hide-scrollbar px-2">
            ...
            <div className="space-y-3">
              ...
              <label className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1 block">Quick Tabs</label>
                {QUICK_TAGS.map(tag => (
                  <button key={tag} onClick={() => toggleTag(tag)} className={`text-[10px] px-2 py-2 gap-1 rounded-full border transition-colors ${wizardForm.tags.includes(tag) ? 'bg-pink-500 border-pink-500 text-white' : 'bg-slate-900 border-slate-700 text-slate-400 hover:border-slate-500'}`}>
                    + {tag}
                  </button>
                ))}

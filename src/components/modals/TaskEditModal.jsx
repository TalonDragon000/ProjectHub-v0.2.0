...
export default function TaskEditModal({ task, onCancel, onSave }) {
  const { updateTask } = useApp(); // ← needs to be added to AppContext (see note below)
  ...
  const handleSave = () => {
    if (!form.title.trim()) return; // don't save a blank title
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col bg-base/95 backdrop-blur-xl animate-in slide-in-from-bottom-full overflow-hidden">
      ...
      {/* Edit mode indicator — distinct color from view mode */}
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
              ...
              <button
                ...
                className={`text-[10px] px-2 py-1 rounded-full border transition-colors ${
                  form.tags.includes(tag)
                    ? 'bg-accent-primary border-accent-primary text-inverted'
            ...
            {form.tags.filter(t => !QUICK_TAGS.includes(t)).map(tag => (
              <button
                key={tag}
                onClick={() => handleToggleTag(tag)}
                className="text-[10px] px-2 py-1 rounded-full border bg-accent-primary border-accent-primary text-inverted"
              >
                {tag} ×
              </button>
            ))}
